...
The above is for running locally.
The below for runnning on ec2 instance.
"""
import subprocess  # probably for python 3 only.
import json
import datetime
...
def main():
    ...
    """
    # all of the uploadings take about 8 seconds:
    send_json_to_ELK("rootkit.json", "aws_rootkit_scan_1", instance_id, date, "account_id in building", uuid_string ,"rootkit")
    send_json_to_ELK("cves.json", "aws_vuls_cves_scan_1", instance_id, date, "account_id in building", uuid_string ,"vuls")
    send_json_to_ELK("lynis.json", "aws_lynis_scan_1", instance_id, date, "account_id in building", uuid_string ,"lynis")
    # how to see: in kibana -> settings -> index patterns -> create index pattern -> providing the names etc.
